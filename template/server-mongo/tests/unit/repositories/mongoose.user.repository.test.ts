...
describe('MongooseUserRepository', () => {
  ...
  describe('getStats', () => {
    it('should aggregate counts by role and activity via the $facet pipeline', async () => {
      const admin = await createTestUser('repo-stats-1@example.com');
      await createTestUser('repo-stats-2@example.com');
      await userRepository.update(admin.id, { role: 'ADMIN', isActive: false });
      ...
      expect(stats.recentSignups).toBe(2);
    });
  });
});
